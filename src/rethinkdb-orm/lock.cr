...
class RethinkORM::Lock < RethinkORM::Base
  extend Settings
  ...
  # HACK: Necessary as overriding `from_trusted_json` wasn't working
...
class RethinkORM::Connection
  protected def self.lock_connection(connection : RethinkDB::Connection?)
    Lock.__connection = connection
  end

  protected def self.lock_db
    Lock.new("lock_table").synchronize do
      yield
    end
  end
end
